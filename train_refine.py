...
from utils.loss_utils import l1_loss, l1_loss_weighted, l2_loss_weighted, lpips_loss, ssim
...
def scene_reconstruction(
    ...
    if hyper.do:
        from scene.deformation import initialize_weights

        gaussians._deformation.deformation_net.opacity_deform.apply(initialize_weights)
    ...
    max_iter_refine = 15000
    offset_iter = 5000

    if opt.lambda_lpips != 0:
        lpips_model = lpips.LPIPS(net="alex", verbose=False).cuda()
    iter_range = range(first_iter, min(final_iter, first_iter + max_iter_refine))
    progress_bar = tqdm(iter_range, desc=f"Training-{stage}", dynamic_ncols=True)
    first_iter += 1
    video_cams = scene.getVideoCameras()

    with open(os.path.join(dataset.source_path, "prompt.txt"), "r") as f:
        text = f.read().strip()

    def get_camera_weight(camera_center, enable=True):
        if not enable:
            return 1.0
        camera_center_0 = torch.tensor([3.8222912231445623, -1.1790206976436188, 0.0]).to(camera_center)
        camera_distance = torch.norm(camera_center - camera_center_0) / 8  # nomralize
        return torch.sin(torch.pi * camera_distance).clamp(0, 1).item()
    ...
    for iteration in iter_range:
        ...
        gaussians.update_learning_rate(iteration + (offset_iter - first_iter), opt.stop_deform)
        ...
        if not viewpoint_stack:
            ...
            viewpoint_stack_loader = DataLoader(
                viewpoint_stack, batch_size=batch_size, shuffle=True, num_workers=opt.num_workers, collate_fn=list
            )
        ...
        if stage == "fine" and hyper.time_smoothness_weight != 0:
            ...
            tv_loss = gaussians.compute_regulation(
                hyper.time_smoothness_weight, hyper.plane_tv_weight, hyper.l1_time_planes
            )
            loss += tv_loss
        ...
        if opt.lambda_sds != 0:
            from guidance.sdxl_utils import get_sds_loss

            loss_sds = get_sds_loss(torch.clamp(image_tensor, 0.0, 1.0))
            loss += opt.lambda_sds * loss_sds
        if opt.lambda_diffusion != 0:
            from guidance.sdxl_utils import get_diffusion_loss

            # loss_diffusion = get_diffusion_loss(torch.clamp(image_tensor, 0.0, 1.0))
            camera_weight = get_camera_weight(viewpoint_cam.camera_center, enable=False)
            # if camera_weight < 0.7:
            #     continue
            loss_diffusion = camera_weight * get_diffusion_loss(
                torch.clamp(image_tensor, 0.0, 1.0),
                gt_image_tensor.to(image_tensor),
                text=text,
                strength_step=opt.refine_step,
                uid=viewpoint_cam.uid,
            )
            loss += opt.lambda_diffusion * loss_diffusion
        loss += Ll1 * 0
...
@torch.no_grad()
def training_report(
    tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene: Scene, renderFunc, renderArgs, stage
):
    if tb_writer:
        ...
        for config in validation_configs:
            if config["cameras"] and len(config["cameras"]) > 0:
                ...
                for idx, viewpoint in enumerate(config["cameras"]):
                    image = torch.clamp(
                        renderFunc(viewpoint, scene.gaussians, stage=stage, *renderArgs)["render"], 0.0, 1.0
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[i * 50 for i in range(2000)])
    parser.add_argument(
        "--save_iterations",
        nargs="+",
        type=int,
    ...
    parser.add_argument(
        "--checkpoint_iterations", nargs="+", type=int, default=[3000, 6000, 10000, 20000, 30000, 60000]
    )
    ...
    if os.path.isdir(args.start_checkpoint):
        ckpts = sorted(glob(os.path.join(args.start_checkpoint, "best_*.pth")))
        if ckpts:
            args.start_checkpoint = ckpts[-1]
        else:
            raise FileNotFoundError(f"No best checkpoint found in {args.start_checkpoint}")
    ...
    if args.model_path:
        os.system(
            f"python render.py --model_path {args.model_path} --configs {args.configs} --iteration -1 --render_ref_time 0 {'--predict_transient' if args.predict_transient else ''} {'--predict_color_deform' if args.predict_color_deform else ''} {'--convert_SHs_python' if args.convert_SHs_python else ''} {'--do' if args.do else ''} {'--gs2d' if args.gs2d else ''}"
        )
