...
class Neural3D_NDC_Dataset(Dataset):
    def __init__(
        ...
        self.eval_index = (eval_index, ) if isinstance(eval_index, int) else eval_index
    ...
    def load_meta(self):
        ...
        videos = sorted(p for p in videos if os.path.isdir(p))
    ...
    def load_images_path(self,videos,split):
        ...
        for index, video_path in enumerate(videos):
            if index in self.eval_index:
                if split =="train":
                    continue
                if split == "test" and index != self.eval_index[0]:
                    continue
            else:
                if split == "test":
                    continue
